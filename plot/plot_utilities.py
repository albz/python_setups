...
def Imshow_threshold( data, gam=1., tvmin=None, tvmax=None, **kwargs ) :
   # "Shows the data with pixel-dependent transparency"
   # "Key word arguments : vmin and vmax set the range of the colormap, "
   # "while threshold is the value below which the system is set to be fully transparent"
    ...
    cmap=kwargs['cmap']
    # Rescale the data to get the transparency and color
    color = (data-tvmin)/(tvmax-tvmin)
    color[color >1.] = 1.
    color[color <0.] = 0.
    # transparency = tmax*(data-tvmin)/(tvmax-tvmin)
    transparency = data*0.
    transparency[data >tvmax] = 1
    transparency[data <tvmin] = 1
    # Application of a gamma
    transparency = tmax*transparency**gam
